...
seconds_per_target="${PATHOLOG_FUZZ_SECONDS:-900}"
targets_raw="${PATHOLOG_FUZZ_TARGETS:-path_clean path_parse_doctor cli_read_only}"
extra_args_raw="${PATHOLOG_FUZZ_ARGS:-}"
cleanup_on_success="${PATHOLOG_FUZZ_CLEANUP:-0}"

read -r -a targets <<< "$targets_raw"
read -r -a extra_args <<< "$extra_args_raw"

if ! [[ "$seconds_per_target" =~ ^[0-9]+$ ]] || [[ "$seconds_per_target" -le 0 ]]; then
  echo "error: PATHOLOG_FUZZ_SECONDS must be a positive integer, got '$seconds_per_target'" >&2
  exit 1
fi
...
for target in "${targets[@]}"; do
  case "$target" in
    path_clean|path_parse_doctor|cli_read_only) ;;
    *)
      echo "error: unsupported fuzz target '$target'" >&2
      echo "supported targets: path_clean path_parse_doctor cli_read_only" >&2
      exit 1
      ;;
  esac
done

tmp_root="$(mktemp -d /tmp/patholog_fuzz_soak.XXXXXX)"
success=0
